			...
			else
			{
				...
				if(!worker_in_ctx && !size_ctxs)
				{
					...
					else if(arch == STARPU_CPU_WORKER)
					{
						...
						{
							double transfer_velocity = starpu_transfer_bandwidth(starpu_worker_get_memory_node(worker), 0);
							transfer_time += (tp->data_size / transfer_velocity) / 1000. ;
							double latency = starpu_transfer_latency(starpu_worker_get_memory_node(worker), 0);
							transfer_time += latency / 1000.;
						}
					}
				}
