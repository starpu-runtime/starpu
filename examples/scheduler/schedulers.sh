...
{
    if [ $1 -eq 77 ] ; then
	( echo SKIP: STARPU_SCHED=$sched $basedir/../cholesky/cholesky_tag >&9 ) 2> /dev/null || true
	echo "skip" >&2
	exit $1
    elif [ $1 -ne 0 ] ; then
...
run()
{
    sched=$1
    echo "cholesky.$sched"
    STARPU_SCHED=$sched $STARPU_SUB_PARALLEL $STARPU_MS_LAUNCHER $STARPU_LOADER $basedir/../cholesky/cholesky_tag -size $(($SIDE*3)) -nblocks 3
    check_success $?
}
