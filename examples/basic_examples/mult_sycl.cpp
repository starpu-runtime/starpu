 ...
 */
static void sycl_mult_kernel(uint32_t nxC, uint32_t nyC, uint32_t nyA,
				       ...
				       const sycl::nd_item<3> &item_ct1)
{
	uint32_t id, i, j, k;
	float sum;
        id = item_ct1.get_group(2) * item_ct1.get_local_range(2) +
...
extern "C" void sycl_mult(void *descr[], void *arg)
{
	(void)arg;
	...
	nblocks = (nxC * nyC + THREADS_PER_BLOCK - 1)/THREADS_PER_BLOCK;
        ...
        try
	{
		starpu_sycl_get_local_stream()->parallel_for(sycl::nd_range<3>(sycl::range<3>(1, 1, nblocks) *
									       sycl::range<3>(1, 1, THREADS_PER_BLOCK),
									       sycl::range<3>(1, 1, THREADS_PER_BLOCK)),
							     [=](sycl::nd_item<3> item_ct1)
							     {
								     sycl_mult_kernel(nxC, nyC, nyA, ldA, ldB, ldC, d_subA,
										      d_subB, d_subC, item_ct1);
							     });
        }
