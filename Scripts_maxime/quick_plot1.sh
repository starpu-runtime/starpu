...
#~ algo1="DARTS+LUF+3D-no-prio push top"
#~ algo2="DARTS+LUF+3D+NTO-no-prio push top"
#~ algo3="DARTS+LUF+3D+NTO+NDO-no-prio push top"
#~ algo4="DARTS+LUF+3D-no-prio push middle"
#~ algo5="DARTS+LUF+3D+NTO-no-prio push middle"
#~ algo6="DARTS+LUF+3D+NTO+NDO-no-prio push middle"
#~ algo7="DARTS+LUF+3D-prio push middle"
#~ algo8="DARTS+LUF+3D+NTO-prio push middle"
#~ algo9="DARTS+LUF+3D+NTO+NDO-prio push middle"
#~ algo10="DMDAR"
...
echo "N,${algo9},${algo10}" > Output_maxime/Legende.txt
...
#~ do
	#~ N=$((START_X+i*ECHELLE_X))
	#~ FREE_PUSHED_TASK_POSITION=0 TASK_ORDER=2 DATA_ORDER=0 DEPENDANCES=1 PRIO=1 APP=$((APP3D)) SEED=$((N/5)) EVICTION_STRATEGY_DYNAMIC_DATA_AWARE=1 STARPU_SCHED_READY=1 STARPU_SIMGRID_CUDA_MALLOC_COST=0 STARPU_EXPECTED_TRANSFER_TIME_WRITEBACK=1 STARPU_HOSTNAME=${HOST} STARPU_SCHED=dynamic-data-aware STARPU_NTASKS_THRESHOLD=$((TH)) STARPU_CUDA_PIPELINE=$((CP)) STARPU_LIMIT_CUDA_MEM=$((CM)) STARPU_BUS_STATS=1 STARPU_BUS_STATS_FILE="${FICHIER_BUS}" STARPU_MINIMUM_CLEAN_BUFFERS=0 STARPU_TARGET_CLEAN_BUFFERS=0 STARPU_NCPU=0 STARPU_NCUDA=$((NGPU)) STARPU_NOPENCL=0 ./examples/cholesky/cholesky_implicit -size $((960*N)) -nblocks $((N)) -no-prio | tail -n 1 >> ${FICHIER_RAW}
	#~ sed -n '4,'$((NCOMBINAISONS))'p' ${FICHIER_BUS} >> ${FICHIER_RAW_DT}
#~ done
...
#~ do
	#~ N=$((START_X+i*ECHELLE_X))
	#~ FREE_PUSHED_TASK_POSITION=0 TASK_ORDER=2 DATA_ORDER=2 DEPENDANCES=1 PRIO=1 APP=$((APP3D)) SEED=$((N/5)) EVICTION_STRATEGY_DYNAMIC_DATA_AWARE=1 STARPU_SCHED_READY=1 STARPU_SIMGRID_CUDA_MALLOC_COST=0 STARPU_EXPECTED_TRANSFER_TIME_WRITEBACK=1 STARPU_HOSTNAME=${HOST} STARPU_SCHED=dynamic-data-aware STARPU_NTASKS_THRESHOLD=$((TH)) STARPU_CUDA_PIPELINE=$((CP)) STARPU_LIMIT_CUDA_MEM=$((CM)) STARPU_BUS_STATS=1 STARPU_BUS_STATS_FILE="${FICHIER_BUS}" STARPU_MINIMUM_CLEAN_BUFFERS=0 STARPU_TARGET_CLEAN_BUFFERS=0 STARPU_NCPU=0 STARPU_NCUDA=$((NGPU)) STARPU_NOPENCL=0 ./examples/cholesky/cholesky_implicit -size $((960*N)) -nblocks $((N)) -no-prio | tail -n 1 >> ${FICHIER_RAW}
	#~ sed -n '4,'$((NCOMBINAISONS))'p' ${FICHIER_BUS} >> ${FICHIER_RAW_DT}
#~ done
#~ echo "#### ${algo4} ####"
#~ for ((i=1 ; i<=(($NB_TAILLE_TESTE)); i++))
#~ do
	#~ N=$((START_X+i*ECHELLE_X))
	#~ FREE_PUSHED_TASK_POSITION=1 DEPENDANCES=1 PRIO=1 APP=$((APP3D)) SEED=$((N/5)) EVICTION_STRATEGY_DYNAMIC_DATA_AWARE=1 STARPU_SCHED_READY=1 STARPU_SIMGRID_CUDA_MALLOC_COST=0 STARPU_EXPECTED_TRANSFER_TIME_WRITEBACK=1 STARPU_HOSTNAME=${HOST} STARPU_SCHED=dynamic-data-aware STARPU_NTASKS_THRESHOLD=$((TH)) STARPU_CUDA_PIPELINE=$((CP)) STARPU_LIMIT_CUDA_MEM=$((CM)) STARPU_BUS_STATS=1 STARPU_BUS_STATS_FILE="${FICHIER_BUS}" STARPU_MINIMUM_CLEAN_BUFFERS=0 STARPU_TARGET_CLEAN_BUFFERS=0 STARPU_NCPU=0 STARPU_NCUDA=$((NGPU)) STARPU_NOPENCL=0 ./examples/cholesky/cholesky_implicit -size $((960*N)) -nblocks $((N)) -no-prio | tail -n 1 >> ${FICHIER_RAW}
	#~ sed -n '4,'$((NCOMBINAISONS))'p' ${FICHIER_BUS} >> ${FICHIER_RAW_DT}
#~ done
...
#~ do
	#~ N=$((START_X+i*ECHELLE_X))
	#~ FREE_PUSHED_TASK_POSITION=1 TASK_ORDER=2 DATA_ORDER=0 DEPENDANCES=1 PRIO=1 APP=$((APP3D)) SEED=$((N/5)) EVICTION_STRATEGY_DYNAMIC_DATA_AWARE=1 STARPU_SCHED_READY=1 STARPU_SIMGRID_CUDA_MALLOC_COST=0 STARPU_EXPECTED_TRANSFER_TIME_WRITEBACK=1 STARPU_HOSTNAME=${HOST} STARPU_SCHED=dynamic-data-aware STARPU_NTASKS_THRESHOLD=$((TH)) STARPU_CUDA_PIPELINE=$((CP)) STARPU_LIMIT_CUDA_MEM=$((CM)) STARPU_BUS_STATS=1 STARPU_BUS_STATS_FILE="${FICHIER_BUS}" STARPU_MINIMUM_CLEAN_BUFFERS=0 STARPU_TARGET_CLEAN_BUFFERS=0 STARPU_NCPU=0 STARPU_NCUDA=$((NGPU)) STARPU_NOPENCL=0 ./examples/cholesky/cholesky_implicit -size $((960*N)) -nblocks $((N)) -no-prio | tail -n 1 >> ${FICHIER_RAW}
	#~ sed -n '4,'$((NCOMBINAISONS))'p' ${FICHIER_BUS} >> ${FICHIER_RAW_DT}
#~ done
...
#~ do
	#~ N=$((START_X+i*ECHELLE_X))
	#~ FREE_PUSHED_TASK_POSITION=1 TASK_ORDER=2 DATA_ORDER=2 DEPENDANCES=1 PRIO=1 APP=$((APP3D)) SEED=$((N/5)) EVICTION_STRATEGY_DYNAMIC_DATA_AWARE=1 STARPU_SCHED_READY=1 STARPU_SIMGRID_CUDA_MALLOC_COST=0 STARPU_EXPECTED_TRANSFER_TIME_WRITEBACK=1 STARPU_HOSTNAME=${HOST} STARPU_SCHED=dynamic-data-aware STARPU_NTASKS_THRESHOLD=$((TH)) STARPU_CUDA_PIPELINE=$((CP)) STARPU_LIMIT_CUDA_MEM=$((CM)) STARPU_BUS_STATS=1 STARPU_BUS_STATS_FILE="${FICHIER_BUS}" STARPU_MINIMUM_CLEAN_BUFFERS=0 STARPU_TARGET_CLEAN_BUFFERS=0 STARPU_NCPU=0 STARPU_NCUDA=$((NGPU)) STARPU_NOPENCL=0 ./examples/cholesky/cholesky_implicit -size $((960*N)) -nblocks $((N)) -no-prio | tail -n 1 >> ${FICHIER_RAW}
	#~ sed -n '4,'$((NCOMBINAISONS))'p' ${FICHIER_BUS} >> ${FICHIER_RAW_DT}
#~ done
