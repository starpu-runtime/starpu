...
N=$((10))
STARPU_SCHED=dmdar STARPU_GENERATE_TRACE=1 STARPU_NTASKS_THRESHOLD=30 STARPU_CUDA_PIPELINE=4 STARPU_LIMIT_CUDA_MEM=500 STARPU_MINIMUM_CLEAN_BUFFERS=0 STARPU_TARGET_CLEAN_BUFFERS=0 STARPU_NCPU=0 STARPU_NCUDA=1 STARPU_NOPENCL=0 STARPU_HOSTNAME=gemini-2 ./examples/mult/sgemm -xy $((960*N)) -nblocks $((N)) -iter 1

#~ STARPU_SCHED=HFP STARPU_NTASKS_THRESHOLD=30 READY=0 PRINTF=0 RANDOM_DATA_ACCESS=1 SEED=1 STARPU_CUDA_PIPELINE=4 BELADY=1 ORDER_U=1 STARPU_LIMIT_CUDA_MEM=150 STARPU_MINIMUM_CLEAN_BUFFERS=0 STARPU_TARGET_CLEAN_BUFFERS=0 STARPU_NCPU=0 STARPU_NCUDA=1 STARPU_NOPENCL=0 STARPU_HOSTNAME=attila ./examples/mult/sgemm -xy $((960*N)) -nblocks $((N)) -iter 1
