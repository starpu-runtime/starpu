...
#~ 2>&1 | tee Output_maxime/terminal_output.txt
#~ TASK_ORDER=$((TASK_ORDER)) DATA_ORDER=$((DATA_ORDER)) STARPU_SIMGRID_CUDA_MALLOC_COST=0 STARPU_WATCHDOG_TIMEOUT=10000000 STARPU_WATCHDOG_CRASH=1 PRINT_N=$((N)) DEPENDANCES=0 STARPU_HOSTNAME=${HOST} STARPU_SCHED=dynamic-data-aware STARPU_SCHED_READY=0 EVICTION_STRATEGY_DYNAMIC_DATA_AWARE=1 STARPU_NTASKS_THRESHOLD=$((TH)) STARPU_CUDA_PIPELINE=$((CP)) APP=1 STARPU_LIMIT_CUDA_MEM=$((CM)) STARPU_MINIMUM_CLEAN_BUFFERS=0 STARPU_TARGET_CLEAN_BUFFERS=0 STARPU_NCPU=0 STARPU_NCUDA=$((NGPU)) STARPU_NOPENCL=0 STARPU_BUS_STATS=0 ./examples/mult/sgemm -3d -xy $((960*N)) -nblocks $((N)) -z $((960*N)) -nblocksz $((N)) -iter $((NITER))

# Dmdar
STARPU_SIMGRID_CUDA_MALLOC_COST=0 STARPU_GENERATE_TRACE=0 STARPU_HOSTNAME=${HOST} STARPU_SCHED=dmdar STARPU_NTASKS_THRESHOLD=$((TH)) STARPU_CUDA_PIPELINE=$((CP)) STARPU_LIMIT_CUDA_MEM=$((CM)) STARPU_MINIMUM_CLEAN_BUFFERS=0 STARPU_TARGET_CLEAN_BUFFERS=0 STARPU_NCPU=0 STARPU_NCUDA=$((NGPU)) STARPU_NOPENCL=0 STARPU_BUS_STATS=0 libtool --mode=execute gdb --args ./examples/mult/sgemm -3d -xy $((960*N)) -nblocks $((N)) -z $((960*4)) -nblocksz $((4)) -iter $((NITER)) --cfg=contexts/factory:thread

# HFP
#~ STARPU_SIMGRID_CUDA_MALLOC_COST=0 STARPU_SCHED=HFP FASTER_FIRST_ITERATION=0 STARPU_SCHED_READY=0 BELADY=0 ORDER_U=1 STARPU_NTASKS_THRESHOLD=$((TH)) STARPU_CUDA_PIPELINE=$((CP)) STARPU_LIMIT_CUDA_MEM=$((CM)) STARPU_MINIMUM_CLEAN_BUFFERS=0 STARPU_TARGET_CLEAN_BUFFERS=0 STARPU_NCPU=0 STARPU_NCUDA=1 STARPU_NOPENCL=0 STARPU_HOSTNAME=gemini-1-fgcs STARPU_BUS_STATS=1 ./examples/mult/sgemm -3d -xy $((960*N)) -nblocks $((N)) -z $((960*4)) -nblocksz $((4)) -iter 1
