...
STARPU_WORKER_STATS=1 LIFT_THRESHOLD_MODE=3 STARPU_WORKER_STATS_FILE=Output_maxime/test.txt STARPU_PROFILING=1 NUMBER_OF_TASK_DONE_BEFORE_LIFTING_THRESHOLD=20 CHOOSE_BEST_DATA_THRESHOLD=10 PERCENTAGE_OF_TASK_DONE_BEFORE_LIFTING_THRESHOLD=20 LIFT_THRESHOLD_MODE=4 PRINTF=0 STARPU_GENERATE_TRACE=0 RANDOM_TASK_ORDER=0 SEED=1 STARPU_SCHED=${ORDO} STARPU_SCHED_READY=$((READY)) DATA_POP_POLICY=1 EVICTION_STRATEGY_DYNAMIC_DATA_AWARE=$((EVICTION)) STARPU_NTASKS_THRESHOLD=$((TH)) STARPU_CUDA_PIPELINE=$((CP)) STARPU_LIMIT_CUDA_MEM=$((CM)) STARPU_MINIMUM_CLEAN_BUFFERS=0 STARPU_BUS_STATS=1 STARPU_TARGET_CLEAN_BUFFERS=0 STARPU_NCPU=0 STARPU_NCUDA=$((NGPU)) STARPU_NOPENCL=0 STARPU_HOSTNAME=${HOST} ./examples/mult/sgemm -xy $((960*N)) -nblocks $((N)) -iter 3	
