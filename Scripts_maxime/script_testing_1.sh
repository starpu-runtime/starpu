...
CHOOSE_BEST_DATA_TYPE=2 STARPU_WORKER_STATS=1 STARPU_BUS_STATS=1 STARPU_WORKER_STATS_FILE=Output_maxime/test2.txt STARPU_BUS_STATS_FILE=Output_maxime/test1.txt STARPU_PROFILING=1 PRINTF=0 STARPU_GENERATE_TRACE=0 RANDOM_TASK_ORDER=0 SEED=1 STARPU_SCHED=${ORDO} STARPU_SCHED_READY=$((READY)) DATA_POP_POLICY=1 EVICTION_STRATEGY_DYNAMIC_DATA_AWARE=$((EVICTION)) STARPU_NTASKS_THRESHOLD=$((TH)) STARPU_CUDA_PIPELINE=$((CP)) STARPU_LIMIT_CUDA_MEM=$((CM)) STARPU_MINIMUM_CLEAN_BUFFERS=0 STARPU_TARGET_CLEAN_BUFFERS=0 STARPU_NCPU=0 STARPU_NCUDA=$((NGPU)) STARPU_NOPENCL=0 STARPU_HOSTNAME=${HOST} libtool --mode=execute gdb --args ./examples/mult/sgemm -xy $((960*N)) -nblocks $((N)) -iter 3 2>&1 | tee Output_maxime/terminal_output.txt

#~ echo $((NGPU)) "1 20 1 1 2 0 0" > Output_maxime/hMETIS_parameters.txt 
#~ PRINTF=0 STARPU_GENERATE_TRACE=0 RANDOM_TASK_ORDER=0 SEED=1 STARPU_SCHED=${ORDO} HMETIS=1 STARPU_SCHED_READY=$((READY)) DATA_POP_POLICY=1 EVICTION_STRATEGY_DYNAMIC_DATA_AWARE=$((EVICTION)) STARPU_NTASKS_THRESHOLD=$((TH)) STARPU_CUDA_PIPELINE=$((CP)) STARPU_LIMIT_CUDA_MEM=$((CM)) STARPU_MINIMUM_CLEAN_BUFFERS=0 STARPU_BUS_STATS=1 STARPU_TARGET_CLEAN_BUFFERS=0 STARPU_NCPU=0 STARPU_NCUDA=$((NGPU)) STARPU_NOPENCL=0 STARPU_HOSTNAME=${HOST} ./examples/mult/sgemm -xy $((960*N)) -nblocks $((N)) -iter 1

# Pour voir si je crash sur grid5000
#~ CHOOSE_BEST_DATA_THRESHOLD=0 STARPU_TASK_PROGRESS=0 SEED=1 STARPU_SCHED=dynamic-data-aware STARPU_SCHED_READY=0 DATA_POP_POLICY=1 EVICTION_STRATEGY_DYNAMIC_DATA_AWARE=1 STARPU_NTASKS_THRESHOLD=10 STARPU_CUDA_PIPELINE=5 STARPU_LIMIT_CUDA_MEM=500 STARPU_MINIMUM_CLEAN_BUFFERS=0 STARPU_BUS_STATS=1 STARPU_TARGET_CLEAN_BUFFERS=0 STARPU_NCPU=0 STARPU_NCUDA=2 STARPU_NOPENCL=0 libtool --mode=execute gdb --args ./examples/mult/sgemm -xy $((960*110)) -nblocks 110 -iter 5
