...
elif [ ${MODEL} == "no_prio" ]; then
	NB_ALGO_TESTE=4	
	N=20
	STARPU_CALIBRATE=2 STARPU_SCHED=dmdar STARPU_NTASKS_THRESHOLD=$((TH)) STARPU_CUDA_PIPELINE=$((CP)) STARPU_MINIMUM_CLEAN_BUFFERS=0 STARPU_EXPECTED_TRANSFER_TIME_WRITEBACK=1 STARPU_TARGET_CLEAN_BUFFERS=0 STARPU_NCPU=0 STARPU_NCUDA=8 STARPU_NOPENCL=0 ./examples/cholesky/cholesky_implicit -size $((TAILLE_TUILE*N)) -nblocks $((N))
	echo "#### EAGER ####"
	for ((i=1 ; i<=(($NB_TAILLE_TESTE)); i++))
	do
		N=$((START_X+i*ECHELLE_X))
