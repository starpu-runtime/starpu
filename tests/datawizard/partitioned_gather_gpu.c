...
int main(void)
{
	...
	/* Fetch the whole piece on the GPU, we should be able to use this to gather */
	starpu_data_acquire_on_node(handle, node, STARPU_R);
	starpu_data_release_on_node(handle, node);

	/* We don't want to drop the whole piece */
	memset_cl.modes[0] = STARPU_RW;

	/* Memset on the pieces on the GPU */
	for (i = 0; i < NPARTS; i++)
		starpu_task_insert(&memset_cl, STARPU_EXECUTE_ON_WORKER, gpu, STARPU_RW, handles[i], 0);
