...
/*
 * Test gathering data without home node with asynchronous partitioning, the
 * gathering node being a GPU
 */
...
int main(void)
{
	...
	{
		FPRINTF(stderr, "This application requires a CPU\n");
		starpu_shutdown();
		return STARPU_TEST_SKIPPED;
	}
	...
	starpu_data_set_gathering_node(handle, node);
	...
	struct starpu_vector_interface *subhandle0_interface = starpu_data_get_interface_on_node(handles[0], node);

	starpu_data_acquire_on_node(handle, node, STARPU_R);
	starpu_data_acquire_on_node(handles[0], node, STARPU_R);
	...
	starpu_data_release_on_node(handles[0], node);
