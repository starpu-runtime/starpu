...
#endif
#if 0
#define setSystemErrno()                                                                \
	do {                                                                            \
		fprintf(stderr, "%s:%d: win %d\n", __FILE__, __LINE__, GetLastError()); \
		errno = unixErrno();                                                    \
	}                                                                               \
	while (0)
#define winPthreadAssertWindows(expr)                                                    \
	do {                                                                             \
		if (!(expr))                                                             \
		{                                                                        \
			fprintf(stderr, "%s:%d: %d\n", __FILE__, __LINE__, unixErrno()); \
			return unixErrno();                                              \
		}                                                                        \
	}                                                                                \
	while (0)
#define winPthreadAssertPthread(expr)                                            \
	do {                                                                     \
		int ret = (expr);                                                \
		if (ret)                                                         \
		{                                                                \
...
{
	/* TODO: we could use CreateMutex and ReleaseMutex to support recursivity */
