...
int main(int argc, char *argv[])
{
	...
	int arity = starpu_getenv_number_default("STARPU_MPI_REDUX_ARITY_THRESHOLD", 2);
	...
	if (comm_size <= arity)
	{
		FPRINTF(stderr, "We need more node than redux tree arity.\n");
		starpu_mpi_shutdown();
		return STARPU_TEST_SKIPPED;
	}
	...
	/* The redux node needs to be >= to arity
         * so the bug due to task pruning appears
	 */
	int redux_node = arity;

	starpu_mpi_barrier(MPI_COMM_WORLD);
	if (comm_rank == redux_node)
	{
		a = comm_rank;
		b[comm_rank] = 1.0 / (comm_rank + 1.0);
		FPRINTF(stderr, "[%d] init a = %f\n", comm_rank, a);
	...
	starpu_mpi_data_register(a_h, tag++, redux_node);
	...
	{
		if (1)//comm_rank == redux_node || comm_rank == work_node)
		{
			for(i=0; i < nworkers; i++ )
			{
				starpu_mpi_task_insert(MPI_COMM_WORLD,
						       &mpi_work_cl,
						       STARPU_MPI_REDUX, a_h,
	...
	ret = starpu_mpi_redux_data_prio_tree(MPI_COMM_WORLD, a_h, 0, arity);
	...
	if (comm_rank == redux_node)
	{
		...
		for (work_node = 0; work_node < comm_size ; work_node++)
		{
			double a_tmp = work_node;
			double b_tmp = 1.0 / ( work_node + 1.0);
