...
void _starpu_mpi_isend_irecv_common(struct _starpu_mpi_req *req, enum starpu_data_access_mode mode, int sequential_consistency)
{
	...
	int node = STARPU_ACQUIRE_NO_NODE;
	...
	{
		starpu_data_acquire_on_node_cb_sequential_consistency_sync_jobids(req->data_handle, &req->data_handle, node, mode, _starpu_mpi_soon_callback, _starpu_mpi_acquired_callback, _starpu_mpi_submit_ready_request, (void *)req, 1 /*sequential consistency*/, 1, &req->pre_sync_jobid, &req->post_sync_jobid, req->prio);
	}
	else
	{
		/* post_sync_job_id has already been filled */
		starpu_data_acquire_on_node_cb_sequential_consistency_sync_jobids(req->data_handle, &req->data_handle, node, mode, _starpu_mpi_soon_callback, _starpu_mpi_acquired_callback, _starpu_mpi_submit_ready_request, (void *)req, 0 /*sequential consistency*/, 1, &req->pre_sync_jobid, NULL, req->prio);
	}
}
...
struct _starpu_mpi_req* _starpu_mpi_isend_cache_aware(starpu_data_handle_t data_handle, int dest, starpu_mpi_tag_t data_tag, MPI_Comm comm, unsigned detached, unsigned sync, int prio, void (*callback)(void *), void *_arg, int sequential_consistency, int* cache_flag)
{
	...
	int already_sent = starpu_mpi_cached_send_set_comm(data_handle, dest, comm);
	if (already_sent == 0)
	{
		*cache_flag = 0;
		if (data_tag == -1)
...
int starpu_mpi_get_data_on_node_detached(MPI_Comm comm, starpu_data_handle_t data_handle, int node, void (*callback)(void*), void *arg)
{
	...
	{
		_STARPU_MPI_DEBUG(1, "Migrating data %p from %d to %d\n", data_handle, rank, node);
		int already_sent = starpu_mpi_cached_send_set_comm(data_handle, node, comm);
		if (already_sent == 0)
		{
...
void starpu_mpi_data_set_source(starpu_data_handle_t data, int node, int new_source)
{
	...
	{
		int already_sent = starpu_mpi_cached_send_set_comm(data, new_source, comm);
		if (already_sent == 0)
		{
