...
int main(int argc, char *argv[])
{
	...
	{
		/* Replicating nodes insert their replicating tasks first. */
		if (work_rk != 0)
		{
			starpu_mpi_task_insert(MPI_COMM_WORLD, &repl_work_cl,
					       STARPU_RW | STARPU_MPI_SAME, A_h,
					       STARPU_EXECUTE_ON_NODE, work_rk,
					       0);
		}
	}
	/* If the original node replicates, it must replicate last to send the
	 * original value.
	 */
	starpu_mpi_task_insert(MPI_COMM_WORLD, &repl_work_cl,
			       STARPU_RW | STARPU_MPI_SAME, A_h,
