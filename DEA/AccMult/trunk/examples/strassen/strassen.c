...
void phase_3_callback_function(void *_arg)
{
	...
	if (cnt == 0)
	{
		...
		ASSERT(arg->strassen_iter_callback);
		arg->strassen_iter_callback(arg->argcb);
	}
}
...
void phase_2_callback_function(void *_arg)
{
	...
	// XXX free Ei1 and Ei2 if needed
	
	switch (i) {
		case 0:
			/* C11 += E1 */
			compute_add_sub_op(iter->E1, SELFADD, NULL, iter->C11, phase_3_callback_function, iter);
			break;
		case 1:
			/* C11 += E2 */
			compute_add_sub_op(iter->E2, SELFADD, NULL, iter->C11, phase_3_callback_function, iter);
			/* C22 += E2 */
			compute_add_sub_op(iter->E2, SELFADD, NULL, iter->C22, phase_3_callback_function, iter);
			break;
		case 2:
			/* C22 -= E3 */
			compute_add_sub_op(iter->E3, SELFSUB, NULL, iter->C22, phase_3_callback_function, iter);
			break;
		case 3:
			/* C11 -= E4 */
			compute_add_sub_op(iter->E4, SELFSUB, NULL, iter->C11, phase_3_callback_function, iter);
			/* C12 += E4 */
			compute_add_sub_op(iter->E4, SELFADD, NULL, iter->C12, phase_3_callback_function, iter);
			break;
		case 4:
			/* C12 += E5 */
			compute_add_sub_op(iter->E5, SELFADD, NULL, iter->C12, phase_3_callback_function, iter);
			/* C22 += E5 */
			compute_add_sub_op(iter->E5, SELFADD, NULL, iter->C22, phase_3_callback_function, iter);
			break;
		case 5:
			/* C11 += E6 */
			compute_add_sub_op(iter->E6, SELFADD, NULL, iter->C11, phase_3_callback_function, iter);
			/* C21 += E6 */
			compute_add_sub_op(iter->E6, SELFADD, NULL, iter->C21, phase_3_callback_function, iter);
			break;
		case 6:
			/* C21 += E7 */
			compute_add_sub_op(iter->E7, SELFADD, NULL, iter->C21, phase_3_callback_function, iter);
			/* C22 -= E7 */
			compute_add_sub_op(iter->E7, SELFSUB, NULL, iter->C22, phase_3_callback_function, iter);
			break;
		default:
			ASSERT(0);
	}
...
static void _strassen_phase_2(strassen_iter_state_t *iter, unsigned i)
{
	...
	switch (i) {
		case 0:
			A = iter->E11; B = iter->E12; C = iter->E1;
			break;
		case 1:
			A = iter->E21; B = iter->E22; C = iter->E2;
			break;
		case 2:
			A = iter->E31; B = iter->E32; C = iter->E3;
			break;
		case 3:
			A = iter->E41; B = iter->E42; C = iter->E4;
			break;
		case 4:
			A = iter->E51; B = iter->E52; C = iter->E5;
			break;
		case 5:
			A = iter->E61; B = iter->E62; C = iter->E6;
			break;
		case 6:
			A = iter->E71; B = iter->E72; C = iter->E7;
			break;
		default:
			ASSERT(0);
	}
	...
	// DEBUG XXX
	//compute_add_sub_op(A, MUL, B, C, phase_2_callback_function, phase_2_arg);
...
static void _do_strassen(data_state *A, data_state *B, data_state *C, void (*strassen_iter_callback)(void *), void *argcb)
{
	...
	_strassen_phase_1(iter->B11, ADD, iter->B12, iter->E22, iter, 2);
