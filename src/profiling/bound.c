...
static void new_task(struct _starpu_job *j)
{
	...
	t->cl = j->task->cl;
	t->footprint = _starpu_compute_buffers_footprint(j->task->cl?j->task->cl->model:NULL, STARPU_CPU_DEFAULT, 0, j);
...
void _starpu_bound_record(struct _starpu_job *j)
{
	...
	else
	{
		...
		_starpu_compute_buffers_footprint(j->task->cl?j->task->cl->model:NULL, STARPU_CPU_DEFAULT, 0, j);
