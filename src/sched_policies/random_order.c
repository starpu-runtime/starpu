...
void random_order_insertion(struct random_order_sched_data *a)
{
...
struct my_list* random_order_delete_link(struct random_order_sched_data* a)
{
	...
	if (starpu_task_list_empty(&data->random_list)) {
		if (!starpu_task_list_empty(&data->sched_list)) {
			...
			for (temp_task_1  = starpu_task_list_begin(&data->popped_task_list); temp_task_1 != starpu_task_list_end(&data->popped_task_list); temp_task_1  = temp_task_2) {
				...
				if(do_not_add_more != 0) { random_order_insertion(data); }
				do_not_add_more--;
			}
