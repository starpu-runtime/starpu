...
static int dynamic_data_aware_push_task(struct starpu_sched_component *component, struct starpu_task *task)
{
	...
	for (i = 0; i < Ngpu; i++) 
	{
		if (is_my_task_free(i + 1, task)) /* +1 cause GPU start at 1, 2, etc ... and you have ram on 0 when checking the memory. */
		{
			...
			initialize_task_data_gpu_single_task(task, 0);
	...
	initialize_task_data_gpu_single_task(task, 1);
		...
		//~ {
			...
			//~ {
				...
				//~ {					
					...
					//~ {						
						//~ printf("Will push data %p on GPU %d, is_present = %d.\n", STARPU_TASK_GET_HANDLE(task, j), i, hud->is_present_in_data_not_used_yet[i]);
		...
		for (i = 0; i < Ngpu; i++)
		{
			for (j = 0; j < STARPU_TASK_GET_NBUFFERS(task); j++)
			{
				...
				{					
					...
					{
						#ifdef PRINT
						printf("Will push data %p on GPU %d, is_present = %d.\n", STARPU_TASK_GET_HANDLE(task, j), i, hud->is_present_in_data_not_used_yet[i]); fflush(stdout);
						#endif
						...
						#ifdef PRINT
						printf("%p gets 1 at is_present_in_data_not_used_yet, %d = %d ?\n", STARPU_TASK_GET_HANDLE(task, j), hud->last_iteration_DARTS, iteration_DARTS); fflush(stdout);
						#endif
				...
				{
					#ifdef PRINT
					printf("New struct hud for data %p\n", STARPU_TASK_GET_HANDLE(task, j)); fflush(stdout);
					#endif
					...
					//~ hud->nb_task_in_pulled_task[i] = 0;
					//~ hud->nb_task_in_planned_task[i] = 0;
					//~ hud->last_check_to_choose_from[i] = 0;
					//~ hud->is_present_in_data_not_used_yet[i] = 1;
					for (k = 0; k < Ngpu; k++)
					{
						hud->nb_task_in_pulled_task[k] = 0;
			...
			{	
				...
				//~ {
					...
					//~ {
						...
						//~ priority_max = temp_priority_max;
					//~ }
					//~ else if ((priority_max == temp_priority_max || prio == 0) && task_using_data_list_size(tudl) > task_available_max_1_from_free) /* Then glovbal number of tasks remaining */
					// if (task_using_data_list_size(tudl) > task_available_max_1_from_free) /* Then glovbal number of tasks remaining */
					//~ {
						//~ task_available_max_1_from_free = task_using_data_list_size(tudl);
						//~ handle_popped = e->D;
						//~ priority_max = temp_priority_max;
					...
					if (STARPU_TASK_GET_HANDLE(t2->pointer_to_T, k) != data_on_node[i] && hud_last_check->last_check_to_choose_from[current_gpu - 1] != g->number_data_selection)
					{
						...
						if (number_free_task_max == 0 && app != 0)
						{
							...
							//~ {
								...
								//~ }
								//~ else if ((priority_max == temp_priority_max || prio == 0) && task_using_data_list_size(tudl) > task_available_max_1_from_free) /* Then glovbal number of tasks remaining */
								//~ // if ((priority_max == temp_priority_max || prio == 0) && task_using_data_list_size(tudl) > task_available_max_1_from_free) /* Then glovbal number of tasks remaining */
								//~ {
									//~ task_available_max_1_from_free = task_using_data_list_size(tudl);
									//~ handle_popped = e->D;
									//~ priority_max = temp_priority_max;
						...
						else /* Cas 2D */
						{
							...
							//~ {
								...
								//~ }
								//~ else if ((priority_max == temp_priority_max || prio == 0) && task_using_data_list_size(tudl) > task_available_max)
								// if (priority_max == temp_priority_max && task_using_data_list_size(tudl) > task_available_max)
								//~ {
									//~ task_available_max = task_using_data_list_size(tudl);
									//~ handle_popped = STARPU_TASK_GET_HANDLE(t2->pointer_to_T, k);
									//~ priority_max = temp_priority_max;
