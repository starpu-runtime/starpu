...
int get_nbuffer_without_scratch(struct starpu_task* t)
{
	...
	{
		if (STARPU_TASK_GET_MODE(t, i) & STARPU_NOFOOTPRINT) continue;
		else
		{
			count += 1;
		}
    ...
    for (i = 0; i < Ngpu; i++)
    {
		...
		for (p = pulled_task_list_begin(tab_gpu_pulled_task[i].ptl); p != pulled_task_list_end(tab_gpu_pulled_task[i].ptl); p = pulled_task_list_next(p))
		{
			...
			for (j = 0; j < STARPU_TASK_GET_NBUFFERS(p->pointer_to_pulled_task); j++)
			{
				...
				if (STARPU_TASK_GET_MODE(p->pointer_to_pulled_task, i) & STARPU_NOFOOTPRINT) continue;	
...
//~ int darts_comparator ( const void * first, const void * second ) 
//~ {
...
static int dynamic_data_aware_push_task(struct starpu_sched_component *component, struct starpu_task *task)
{
	...
	for (i = 0; i < STARPU_TASK_GET_NBUFFERS(task); i++)
	{
		if (STARPU_TASK_GET_MODE(task, i) & STARPU_NOFOOTPRINT)
		{
			...
			printf(" %p mode is NOFOOTPRINT", STARPU_TASK_GET_HANDLE(task, i)); fflush(stdout);
		}
		else
		{
	...
	{
		sorted_gpu_list_by_nb_task_in_planned_task = malloc(Ngpu*sizeof(int));
		planned_task_sizes = malloc(Ngpu*sizeof(int));
		for (i = 0; i < Ngpu; i++)
		{
			sorted_gpu_list_by_nb_task_in_planned_task[i] = i;
			if (&tab_gpu_planned_task[i].planned_task == NULL)
			{
				planned_task_sizes[i] = 0;
			}
			else 
			{
	...
	//	if (is_my_task_free(gpu_looked_at, task) || strcmp(starpu_task_get_name(task), "chol_model_11") == 0)
		{
			...
			increment_planned_task_data(task, gpu_looked_at);	
			...
			else
			{
				...
				for (checked_task = starpu_task_list_begin(&tab_gpu_planned_task[gpu_looked_at - 1].planned_task); checked_task != starpu_task_list_end(&tab_gpu_planned_task[gpu_looked_at - 1].planned_task); checked_task = starpu_task_list_next(checked_task))
				{
					for (j = 0; j < STARPU_TASK_GET_NBUFFERS(checked_task); j++)
					{
						...
						if (STARPU_TASK_GET_MODE(checked_task, j) & STARPU_NOFOOTPRINT) continue;	
...
static struct starpu_task *dynamic_data_aware_pull_task(struct starpu_sched_component *component, struct starpu_sched_component *to)
{
	...
	if (task != NULL)
	{
		printf("Pulled task %p on GPU %d.\n", task, starpu_worker_get_memory_node(starpu_worker_get_id())); fflush(stdout);
	}
	#endif
...
void dynamic_data_aware_scheduling_3D_matrix(struct starpu_task_list *main_task_list, int current_gpu, struct gpu_planned_task *g)
{
	...
	if (choose_best_data_from == 0) /* Le cas de base où je regarde les données pas encore utilisées. */
	{
		...
		//~ print_data_not_used_yet_one_gpu(g, 1);
		...
		for (e = gpu_data_not_used_list_begin(g->gpu_data); e != gpu_data_not_used_list_end(g->gpu_data) && i != choose_best_data_threshold; e = gpu_data_not_used_list_next(e), i++)
		{
			...
			if (e->D->sched_data != NULL)
			{
				...
				for (t = task_using_data_list_begin(e->D->sched_data); t != task_using_data_list_end(e->D->sched_data); t = task_using_data_list_next(t))
				{
					...
					{
						if (STARPU_TASK_GET_MODE(t->pointer_to_T, j) & STARPU_NOFOOTPRINT) continue;	
	...
	else if (choose_best_data_from == 1) /* Le cas où je regarde uniquement les données (pas encore en mémoire) des tâches utilisant les données en mémoire. */
	{
		...
		for (i = 0; i < nb_data_on_node; i++)
		{
			...
			for (t2 = task_using_data_list_begin(data_on_node[i]->sched_data); t2 != task_using_data_list_end(data_on_node[i]->sched_data); t2 = task_using_data_list_next(t2))
			{
				...
				{
					if (STARPU_TASK_GET_MODE(t2->pointer_to_T, k) & STARPU_NOFOOTPRINT) continue;	
					#ifdef PRINT
    ...
    if (number_free_task_max != 0) /* Cas comme dans 2D, je met dans planned_task les tâches gratuites, sauf que j'ai 3 données à check et non 2. */
    {
		...
		for (t = task_using_data_list_begin(handle_popped->sched_data); t != task_using_data_list_end(handle_popped->sched_data); t = task_using_data_list_next(t))
		{
			...
			if (data_available == true)
			{
				...
				{
					if (STARPU_TASK_GET_MODE(t->pointer_to_T, i) & STARPU_NOFOOTPRINT) continue;	
	...
	else if (number_1_from_free_task_max != 0 && app != 0) /* On prend une tâche de la donnée 1_from_free, dans l'ordre randomisé de la liste de tâches. 1 seule tâche est ajouté à planned_task. */
	{
		...
		if (choose_best_data_from == 0) /* Que dans le cas où je choisis depuis la liste des pas encore utilisées. */
		{
			...
			{
				if (STARPU_TASK_GET_MODE(best_1_from_free_task, i) & STARPU_NOFOOTPRINT) continue;	
...
starpu_data_handle_t dynamic_data_aware_victim_selector(starpu_data_handle_t toload, unsigned node, enum starpu_is_prefetch is_prefetch, void *component)
{
    ...
    for (i = 0; i < nb_data_on_node; i++)
    {
		...
		{
			//printf("%p on node has a size of %lu\n", data_on_node[i], starpu_data_get_size(data_on_node[i])); fflush(stdout); 
...
{	
	...
	push_free_task_on_gpu_with_least_task_in_planned_task = starpu_get_env_number_default("PUSH_FREE_TASK_ON_GPU_WITH_LEAST_TASK_IN_PLANNED_TASK", 0);

	#ifdef PRINT	
	printf("-----\nEVICTION_STRATEGY_DYNAMIC_DATA_AWARE = %d\nTHRESHOLD = %d\nAPP = %d\nCHOOSE_BEST_DATA_FROM = %d\nSIMULATE_MEMORY = %d\nTASK_ORDER = %d\nDATA_ORDER = %d\nDEPENDANCES = %d\nPRIO = %d\nFREE_PUSHED_TASK_POSITION = %d\nGRAPH_DESCENDANTS = %d\nDOPT_SELECTION_ORDER = %d\nPRIORITY_ATTRIBUTION = %d\nHIGHEST_PRIORITY_TASK_RETURNED_IN_DEFAULT_CASE = %d\nCAN_A_DATA_BE_IN_MEM_AND_IN_NOT_USED_YET = %d\nPUSH_FREE_TASK_ON_GPU_WITH_LEAST_TASK_IN_PLANNED_TASK = %d\n-----\n", eviction_strategy_dynamic_data_aware, threshold, app, choose_best_data_from, simulate_memory, task_order, data_order, dependances, prio, free_pushed_task_position, graph_descendants, dopt_selection_order, starpu_get_env_number_default("PRIORITY_ATTRIBUTION", 0), highest_priority_task_returned_in_default_case, can_a_data_be_in_mem_and_in_not_used_yet, push_free_task_on_gpu_with_least_task_in_planned_task);
	#endif
