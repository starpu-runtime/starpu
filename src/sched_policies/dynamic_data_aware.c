...
int get_nbuffer_without_scratch(struct starpu_task* t)
{
	...
	{
		if ((STARPU_TASK_GET_MODE(t, i) & STARPU_SCRATCH) || (STARPU_TASK_GET_MODE(t, i) & STARPU_REDUX)) continue;
		else
		{
			count += 1;
		}
...
{
//	printf("Add task to pulled task\n"); fflush(stdout);
	int i = 0;	
...
{	
	...
	cpu_only = starpu_get_env_number_default("CPU_ONLY", 0);
	#ifdef PRINT	
