...
#define MULTIGPU /* 0 : on ne fais rien, 1 : on construit |GPU| paquets et on attribue chaque paquet à un GPU au hasard, 2 : pareil que 1 + load balance */
...
//~ struct paquets
//~ {	
	//~ struct listes *listes_pointer_1;
	//~ struct listes *listes_pointer_2;
	//~ struct listes *listes_first_link;     	
    //~ int NP;
//~ };
...
static void get_ordre_utilisation_donnee(struct HFP_sched_data* a, int NB_TOTAL_DONNEES, int nb_gpu)
{
	...
	summed_nb_data_each_gpu = malloc(nb_gpu*sizeof(int));
	summed_nb_task_each_gpu = malloc(nb_gpu*sizeof(int));
	data_use_order = malloc(NB_TOTAL_DONNEES*sizeof(a->temp_pointer_1->package_data[0]));
	while (a->temp_pointer_1 != NULL) {
		//~ j = 0;
		if (l == 0) { 
			summed_nb_data_each_gpu[l] = a->temp_pointer_1->total_nb_data_package;
			summed_nb_task_each_gpu[l] = a->temp_pointer_1->nb_task_in_sub_list;
		}
		else {
			summed_nb_data_each_gpu[l] = a->temp_pointer_1->total_nb_data_package + summed_nb_data_each_gpu[l-1];
			summed_nb_task_each_gpu[l] = a->temp_pointer_1->nb_task_in_sub_list + summed_nb_task_each_gpu[l-1];
		}
		l++;
		...
		//~ data_use_order = data_use_order->next_gpu;
		fprintf(f_2,"-------------\n");
		fprintf(f,"-------------\n");
	}
	fclose(f);
...
static struct starpu_task *get_task_to_return(struct starpu_sched_component *component, struct starpu_sched_component *to, struct HFP_sched_data* a)
{
	int i = 0;
...
void print_packages_in_terminal (struct HFP_sched_data *a, int nb_of_loop) {
	int link_index = 0; int i = 0;
	...
	while (a->temp_pointer_1 != NULL) { link_index++; a->temp_pointer_1 = a->temp_pointer_1->next;				
			...
			link_index = 0;	
			while (a->temp_pointer_1 != NULL) {
				printf("Le paquet %d contient %d tâche(s) et %d données\n",link_index,a->temp_pointer_1->nb_task_in_sub_list,a->temp_pointer_1->package_nb_data);
...
struct starpu_task_list hierarchical_fair_packing (struct starpu_task_list task_list, int number_task, starpu_ssize_t GPU_RAM_M) {
	...
	//~ struct listes *listes_data = malloc(sizeof(*listes_data));
	//~ starpu_task_list_init(&listes_data->sub_list);
 	//~ listes_data->next = NULL;
	//~ paquets_data->listes_pointer_1 = listes_data;
	...
	//~ int bool_data_common = 0; int GPU_limit_switch = 0; int temp_nb_min_task_packages = 0; int i_bis = 0; int j_bis = 0; int j = 0; int tab_runner = 0; int index_head_1 = 0; int index_head_2 = 0; int common_data_last_package_i2_j = 0; int common_data_last_package_i1_j = 0; int common_data_last_package_i_j1 = 0; int common_data_last_package_i_j2 = 0;
	...
	//~ paquets_data->listes_pointer_1->package_data = malloc(STARPU_TASK_GET_NBUFFERS(task)*sizeof(paquets_data->listes_pointer_1->package_data[0]));
	//~ struct starpu_task *temp_task;
			...
			//~ for (task = starpu_task_list_begin(&task_list); task != starpu_task_list_end(&task_list); task = temp_task) {
				...
				//~ for (i = 0; i < STARPU_TASK_GET_NBUFFERS(task); i++) {
					//~ paquets_data->listes_pointer_1->package_data[i] = STARPU_TASK_GET_HANDLE(task,i);
				//~ }
				//~ paquets_data->listes_pointer_1->package_nb_data = STARPU_TASK_GET_NBUFFERS(task);
				//~ NB_TOTAL_DONNEES+=STARPU_TASK_GET_NBUFFERS(task);
				//~ /* We sort our datas in the packages */
				//~ qsort(paquets_data->listes_pointer_1->package_data,paquets_data->listes_pointer_1->package_nb_data,sizeof(paquets_data->listes_pointer_1->package_data[0]),HFP_pointeurComparator);
				//~ /* Pushing the task and the number of the package in the package*/
				//~ starpu_task_list_push_back(&paquets_data->listes_pointer_1->sub_list,task);
				//~ paquets_data->listes_pointer_1->index_package = link_index;
				//~ /* Initialization of the lists last_packages */
				//~ paquets_data->listes_pointer_1->split_last_ij = 0;
				//~ paquets_data->listes_pointer_1->total_nb_data_package = STARPU_TASK_GET_NBUFFERS(task);
				//~ link_index++;
				//~ paquets_data->listes_pointer_1->nb_task_in_sub_list=1;
				
				//~ if(do_not_add_more != 0) { 
					//~ HFP_insertion(paquets_data); paquets_data->listes_pointer_1->package_data = malloc(STARPU_TASK_GET_NBUFFERS(task)*sizeof(paquets_data->listes_pointer_1->package_data[0])); 
				//~ }
				//~ do_not_add_more--;
			//~ }
			//~ paquets_data->listes_first_link = paquets_data->listes_pointer_1;
			//~ paquets_data->listes_pointer_2 = paquets_data->listes_first_link;
			//~ index_head_2++;
			...
			//~ print_packages_in_terminal(paquets_data, nb_of_loop);
			...
			//~ while (packaging_impossible == 0) {
				...
				//~ paquets_data->listes_pointer_1 = paquets_data->listes_first_link; paquets_data->listes_pointer_2 = paquets_data->listes_first_link; index_head_1 = 0; index_head_2 = 1; link_index = 0; tab_runner = 0; nb_min_task_packages = 0;
				...
				//~ min_nb_task_in_sub_list = paquets_data->listes_pointer_1->nb_task_in_sub_list; for (i = 0; i < number_task; i++) { for (j = 0; j < number_task; j++) { matrice_donnees_commune[i][j] = 0; }}
								 
					//~ /* First we get the number of packages that have the minimal number of tasks */
					//~ for (paquets_data->listes_pointer_1 = paquets_data->listes_first_link; paquets_data->listes_pointer_1 != NULL; paquets_data->listes_pointer_1 = paquets_data->listes_pointer_1->next) {
						//~ if (min_nb_task_in_sub_list > paquets_data->listes_pointer_1->nb_task_in_sub_list) { min_nb_task_in_sub_list = paquets_data->listes_pointer_1->nb_task_in_sub_list; } }
					//~ for (paquets_data->listes_pointer_1 = paquets_data->listes_first_link; paquets_data->listes_pointer_1 != NULL; paquets_data->listes_pointer_1 = paquets_data->listes_pointer_1->next) {
						//~ if (min_nb_task_in_sub_list == paquets_data->listes_pointer_1->nb_task_in_sub_list) { nb_min_task_packages++; } }
					...
					//~ for (paquets_data->listes_pointer_1 = paquets_data->listes_first_link; paquets_data->listes_pointer_1 != NULL; paquets_data->listes_pointer_1 = paquets_data->listes_pointer_1->next) {
						//~ for (paquets_data->listes_pointer_2 = paquets_data->listes_pointer_1->next; paquets_data->listes_pointer_2 != NULL; paquets_data->listes_pointer_2 = paquets_data->listes_pointer_2->next) {
							//~ for (i = 0; i < paquets_data->listes_pointer_1->package_nb_data; i++) {
								//~ for (j = 0; j < paquets_data->listes_pointer_2->package_nb_data; j++) {
									//~ if ((paquets_data->listes_pointer_1->package_data[i] == paquets_data->listes_pointer_2->package_data[j])) {
										//~ matrice_donnees_commune[index_head_1][index_head_2] += starpu_data_get_size(paquets_data->listes_pointer_2->package_data[j]) + starpu_data_get_size(paquets_data->listes_pointer_1->package_data[i]);
										//~ matrice_donnees_commune[index_head_2][index_head_1] += starpu_data_get_size(paquets_data->listes_pointer_2->package_data[j]) + starpu_data_get_size(paquets_data->listes_pointer_1->package_data[i]);
				...
				//~ paquets_data->listes_pointer_1 = paquets_data->listes_first_link; paquets_data->listes_pointer_2 = paquets_data->listes_first_link;
				
					//~ i_bis = 0; j_bis = 0; 
					//~ temp_nb_min_task_packages = nb_min_task_packages;
				//~ debut_while:
					//~ paquets_data->listes_pointer_1 = paquets_data->listes_first_link;
					//~ paquets_data->listes_pointer_2 = paquets_data->listes_first_link;
					//~ max_value_common_data_matrix = 0;
					//~ if (GPU_limit_switch == 1) {
					//~ for (i_bis = 0; i_bis < number_task; i_bis++) {
						//~ if (paquets_data->listes_pointer_1->nb_task_in_sub_list == min_nb_task_in_sub_list) { //Si on est sur un paquet de taille minimale
							//~ for (paquets_data->listes_pointer_2 = paquets_data->listes_first_link; paquets_data->listes_pointer_2 != NULL; paquets_data->listes_pointer_2 = paquets_data->listes_pointer_2->next) {
								//~ if (i_bis != j_bis) {
									//~ weight_two_packages = 0;
									//~ for (i = 0; i < paquets_data->listes_pointer_1->package_nb_data; i++) { weight_two_packages += starpu_data_get_size(paquets_data->listes_pointer_1->package_data[i]); } 
									//~ for (i = 0; i < paquets_data->listes_pointer_2->package_nb_data; i++) {
										//~ bool_data_common = 0;
										//~ for (j = 0; j < paquets_data->listes_pointer_1->package_nb_data; j++) {
										//~ if (paquets_data->listes_pointer_2->package_data[i] == paquets_data->listes_pointer_1->package_data[j]) { bool_data_common = 1; } }
										//~ if (bool_data_common != 1) { weight_two_packages += starpu_data_get_size(paquets_data->listes_pointer_2->package_data[i]); } } 
							...
							//~ } j_bis++; } tab_runner++; } 
							//~ paquets_data->listes_pointer_1=paquets_data->listes_pointer_1->next;
							//~ j_bis = 0; }
				//~ paquets_data->listes_pointer_1 = paquets_data->listes_first_link; paquets_data->listes_pointer_2 = paquets_data->listes_first_link;
				//~ }
				...
				//~ for (i = 0; i < number_task; i++) {
					//~ if (paquets_data->listes_pointer_1->nb_task_in_sub_list == min_nb_task_in_sub_list) {
						//~ for (j = 0; j < number_task; j++) {
							//~ weight_two_packages = 0;  weight_package_i = 0;  weight_package_j = 0;
							//~ for (i_bis = 0; i_bis < paquets_data->listes_pointer_1->package_nb_data; i_bis++) { weight_two_packages += starpu_data_get_size(paquets_data->listes_pointer_1->package_data[i_bis]); } weight_package_i = weight_two_packages;
							//~ for (i_bis = 0; i_bis < paquets_data->listes_pointer_2->package_nb_data; i_bis++) { bool_data_common = 0;
								//~ for (j_bis = 0; j_bis < paquets_data->listes_pointer_1->package_nb_data; j_bis++) { if (paquets_data->listes_pointer_2->package_data[i_bis] == paquets_data->listes_pointer_1->package_data[j_bis]) { bool_data_common = 1; } }
								//~ if (bool_data_common != 1) { weight_two_packages += starpu_data_get_size(paquets_data->listes_pointer_2->package_data[i_bis]); } 
								//~ weight_package_j += starpu_data_get_size(paquets_data->listes_pointer_2->package_data[i_bis]); }							
							//~ if (matrice_donnees_commune[i][j] == max_value_common_data_matrix && i != j && max_value_common_data_matrix != 0) {
								...
								//~ if (paquets_data->listes_pointer_2->nb_task_in_sub_list == min_nb_task_in_sub_list) { temp_nb_min_task_packages--; }
								...
								//~ if (starpu_get_env_number_default("ORDER_U",0) == 1) {
									//~ if (paquets_data->listes_pointer_1->nb_task_in_sub_list == 1 && paquets_data->listes_pointer_2->nb_task_in_sub_list == 1) {
									...
									//~ else if (weight_package_i > GPU_RAM_M && weight_package_j <= GPU_RAM_M) {
										...
										//~ common_data_last_package_i1_j = get_common_data_last_package(paquets_data->listes_pointer_1, paquets_data->listes_pointer_2, 1, 0, false,GPU_RAM_M);					
										//~ common_data_last_package_i2_j = get_common_data_last_package(paquets_data->listes_pointer_1, paquets_data->listes_pointer_2, 2, 0, false,GPU_RAM_M);					
										...
										//~ if (common_data_last_package_i1_j > common_data_last_package_i2_j) {
											...
											//~ paquets_data->listes_pointer_1 = HFP_reverse_sub_list(paquets_data->listes_pointer_1);
										//~ }
									//~ }
									//~ else if (weight_package_i <= GPU_RAM_M && weight_package_j > GPU_RAM_M) {
										//~ common_data_last_package_i_j1 = get_common_data_last_package(paquets_data->listes_pointer_1, paquets_data->listes_pointer_2, 0, 1, false,GPU_RAM_M);					
										//~ common_data_last_package_i_j2 = get_common_data_last_package(paquets_data->listes_pointer_1, paquets_data->listes_pointer_2, 0, 2, false,GPU_RAM_M);					
										//~ if (common_data_last_package_i_j2 > common_data_last_package_i_j1) {
											...
											//~ paquets_data->listes_pointer_2 = HFP_reverse_sub_list(paquets_data->listes_pointer_2);
										//~ }
									//~ }
									//~ else {
										//~ if (weight_package_i > GPU_RAM_M && weight_package_j > GPU_RAM_M) {
											//~ common_data_last_package_i1_j1 = get_common_data_last_package(paquets_data->listes_pointer_1, paquets_data->listes_pointer_2, 1, 1, false,GPU_RAM_M);					
											//~ common_data_last_package_i1_j2 = get_common_data_last_package(paquets_data->listes_pointer_1, paquets_data->listes_pointer_2, 1, 2, false,GPU_RAM_M);
											//~ common_data_last_package_i2_j1 = get_common_data_last_package(paquets_data->listes_pointer_1, paquets_data->listes_pointer_2, 2, 1, false,GPU_RAM_M);					
											//~ common_data_last_package_i2_j2 = get_common_data_last_package(paquets_data->listes_pointer_1, paquets_data->listes_pointer_2, 2, 2, false,GPU_RAM_M);
										//~ }
										//~ else if (weight_package_i <= GPU_RAM_M && weight_package_j <= GPU_RAM_M) {
											//~ if (starpu_get_env_number_default("PRINTF",0) == 1) { printf("I <= PU_RAM et J <= PU_RAM\n"); }
											//~ common_data_last_package_i1_j1 = get_common_data_last_package(paquets_data->listes_pointer_1, paquets_data->listes_pointer_2, 1, 1, true,GPU_RAM_M);					
											//~ common_data_last_package_i1_j2 = get_common_data_last_package(paquets_data->listes_pointer_1, paquets_data->listes_pointer_2, 1, 2, true,GPU_RAM_M);
											//~ common_data_last_package_i2_j1 = get_common_data_last_package(paquets_data->listes_pointer_1, paquets_data->listes_pointer_2, 2, 1, true,GPU_RAM_M);					
											//~ common_data_last_package_i2_j2 = get_common_data_last_package(paquets_data->listes_pointer_1, paquets_data->listes_pointer_2, 2, 2, true,GPU_RAM_M);
										//~ }
								...
								//~ paquets_data->listes_pointer_1->split_last_ij = paquets_data->listes_pointer_1->nb_task_in_sub_list;
								//~ while (!starpu_task_list_empty(&paquets_data->listes_pointer_2->sub_list)) {
								//~ starpu_task_list_push_back(&paquets_data->listes_pointer_1->sub_list,starpu_task_list_pop_front(&paquets_data->listes_pointer_2->sub_list)); 
								//~ paquets_data->listes_pointer_1->nb_task_in_sub_list ++; }
								//~ i_bis = 0; j_bis = 0; tab_runner = 0;
								//~ starpu_data_handle_t *temp_data_tab = malloc((paquets_data->listes_pointer_1->package_nb_data + paquets_data->listes_pointer_2->package_nb_data) * sizeof(paquets_data->listes_pointer_1->package_data[0]));
								//~ while (i_bis < paquets_data->listes_pointer_1->package_nb_data && j_bis < paquets_data->listes_pointer_2->package_nb_data) {
									//~ if (paquets_data->listes_pointer_1->package_data[i_bis] <= paquets_data->listes_pointer_2->package_data[j_bis]) {
										//~ temp_data_tab[tab_runner] = paquets_data->listes_pointer_1->package_data[i_bis];
										//~ i_bis++; }
									//~ else {
										//~ temp_data_tab[tab_runner] = paquets_data->listes_pointer_2->package_data[j_bis];
										//~ j_bis++; }
									//~ tab_runner++;
								//~ }
								//~ while (i_bis < paquets_data->listes_pointer_1->package_nb_data) { temp_data_tab[tab_runner] = paquets_data->listes_pointer_1->package_data[i_bis]; i_bis++; tab_runner++; }
								//~ while (j_bis < paquets_data->listes_pointer_2->package_nb_data) { temp_data_tab[tab_runner] = paquets_data->listes_pointer_2->package_data[j_bis]; j_bis++; tab_runner++; }
								//~ for (i_bis = 0; i_bis < (paquets_data->listes_pointer_1->package_nb_data + paquets_data->listes_pointer_2->package_nb_data); i_bis++) {
									//~ if (temp_data_tab[i_bis] == temp_data_tab[i_bis + 1]) {
										//~ temp_data_tab[i_bis] = 0;
										//~ nb_duplicate_data++; } }
								//~ paquets_data->listes_pointer_1->package_data = malloc((paquets_data->listes_pointer_1->package_nb_data + paquets_data->listes_pointer_2->package_nb_data - nb_duplicate_data) * sizeof(starpu_data_handle_t));
								//~ j_bis = 0;
								//~ for (i_bis = 0; i_bis < (paquets_data->listes_pointer_1->package_nb_data + paquets_data->listes_pointer_2->package_nb_data); i_bis++) {
									//~ if (temp_data_tab[i_bis] != 0) { paquets_data->listes_pointer_1->package_data[j_bis] = temp_data_tab[i_bis]; j_bis++; } }
								//~ paquets_data->listes_pointer_1->package_nb_data = paquets_data->listes_pointer_2->package_nb_data + paquets_data->listes_pointer_1->package_nb_data - nb_duplicate_data;
								
								//~ paquets_data->listes_pointer_1->total_nb_data_package += paquets_data->listes_pointer_2->total_nb_data_package;
								
								//~ paquets_data->listes_pointer_2->package_nb_data = 0;
								//~ nb_duplicate_data = 0;
								//~ paquets_data->listes_pointer_2->nb_task_in_sub_list = 0;
							...
							//~ } }
							//~ paquets_data->listes_pointer_2=paquets_data->listes_pointer_2->next;
						//~ }
					//~ }
					//~ paquets_data->listes_pointer_1=paquets_data->listes_pointer_1->next; paquets_data->listes_pointer_2=paquets_data->listes_first_link;
				//~ }			
				
				//~ break_merging_1:
				
				//~ paquets_data->listes_pointer_1 = paquets_data->listes_first_link;
				//~ paquets_data->listes_pointer_1 = HFP_delete_link(paquets_data);
				//~ tab_runner = 0;
				
				//~ /* Code to get the coordinates of each data in the order in wich tasks get out of pull_task */
					//~ while (paquets_data->listes_pointer_1 != NULL) {
					//~ /* if ((strcmp(appli,"starpu_sgemm_gemm") == 0) && (starpu_get_env_number_default("PRINTF",0) == 1)) {
						//~ for (task = starpu_task_list_begin(&paquets_data->listes_pointer_1->sub_list); task != starpu_task_list_end(&paquets_data->listes_pointer_1->sub_list); task  = starpu_task_list_next(task)) {
							//~ starpu_data_get_coordinates_array(STARPU_TASK_GET_HANDLE(task,2),2,temp_tab_coordinates);
							//~ coordinate_visualization_matrix[temp_tab_coordinates[0]][temp_tab_coordinates[1]] = NT - paquets_data->listes_pointer_1->index_package - 1;
		...
		//~ return paquets_data->listes_first_link->sub_list;
}
 ...
 * TODO : It is necessary to re-apply HFP on each of these packages afterward
 * TODO : It would be good to have the package building in a separete function in order to do that
 */
void load_balance (struct HFP_sched_data *a, int number_gpu)
{
	...
	printf("A package should have %d or %d tasks\n", NT/number_gpu, NT/number_gpu+1);
	...
	while (load_balance_needed == true) { 
		...
		if (package_with_min_number_task == package_with_max_number_task || min_number_task_in_package ==  max_number_task_in_package-1) {
			printf("All packages have the same number of tasks +/- 1 task\n");
...
static struct starpu_task *HFP_pull_task(struct starpu_sched_component *component, struct starpu_sched_component *to)
{
	...
	if (is_empty(data->first_link) == true) {
		//~ printf("is empty a renvoyé vrai\n");
		if (!starpu_task_list_empty(&data->sched_list)) { /* Si la liste initiale (sched_list) n'est pas vide, ce sont des tâches non traitées */
			...
			for (temp_task_1  = starpu_task_list_begin(&data->popped_task_list); temp_task_1 != starpu_task_list_end(&data->popped_task_list); temp_task_1  = temp_task_2) {
				...
				data->temp_pointer_1->total_nb_data_package = STARPU_TASK_GET_NBUFFERS(temp_task_1);
					...
					if (data->temp_pointer_1->nb_task_in_sub_list == min_nb_task_in_sub_list) {
						for (j = 0; j < nb_pop; j++) {
							...
							if (matrice_donnees_commune[i][j] == max_value_common_data_matrix && i != j && max_value_common_data_matrix != 0) {
								...
								data->temp_pointer_1->total_nb_data_package += data->temp_pointer_2->total_nb_data_package;
								
								data->temp_pointer_2->package_nb_data = 0;
								nb_duplicate_data = 0;
								data->temp_pointer_2->nb_task_in_sub_list = 0;
							temp_nb_min_task_packages--;
							if(data->NP == number_of_package_to_build) { goto break_merging; }
							if (temp_nb_min_task_packages > 1) {
								goto debut_while; 
							}
		...
		if (starpu_get_env_number_default("PRINTF",0) == 1) { print_packages_in_terminal(data, nb_of_loop); }
		
		/* Task stealing. Only in cases of multigpu */
		if (starpu_get_env_number_default("MULTIGPU",0) == 2) {
			load_balance(data, component->nchildren);
			if (starpu_get_env_number_default("PRINTF",0) == 1) { print_packages_in_terminal(data, nb_of_loop); }
		}
		 ...
		 */
		 if (starpu_get_env_number_default("MULTIGPU",0) != 0) {
			 //TODO : itérer sur tous les paquets
			 //TODO : need to return a list so we can empty the list in data and reput task in it 
			 //~ data->first_link->sub_list = hierarchical_fair_packing(data->first_link->sub_list, data->first_link->nb_task_in_sub_list, GPU_RAM_M);
			 //~ if (starpu_get_env_number_default("PRINTF",0) == 1) { print_packages_in_terminal(data, nb_of_loop); }
		 }
		...
		if (starpu_get_env_number_default("BELADY",0) == 1) {
			get_ordre_utilisation_donnee(data, NB_TOTAL_DONNEES, component->nchildren);
		}
		...
		task1 = get_task_to_return(component, to, data);
		...
		/* Else de if (!starpu_task_list_empty(&data->sched_list)), il faut donc return une tâche */
			...
			//~ if (starpu_get_env_number_default("PRINTF",0) == 1) { printf("Task %p is getting out of pull_task from gpu %p\n",task1,to); }			
			return task1; /* Renvoie nil içi */
	} /* End of if ((data->temp_pointer_1->next == NULL) && (starpu_task_list_empty(&data->temp_pointer_1->sub_list))) { */
		task1 = get_task_to_return(component, to, data);
		if (starpu_get_env_number_default("PRINTF",0) == 1 && task1 != NULL) { printf("Task %p is getting out of pull_task from gpu %p\n",task1,to); }
		//~ if (starpu_get_env_number_default("PRINTF",0) == 1) { printf("Task %p is getting out of pull_task from gpu %p\n",task1,to); }
		STARPU_PTHREAD_MUTEX_UNLOCK(&data->policy_mutex);
...
starpu_data_handle_t belady_victim_selector(starpu_data_handle_t toload, unsigned node, enum starpu_is_prefetch is_prefetch)
{
	...
	int is_allocated;
	//~ struct data_use_order = struct gpu_list->first_gpu;
	...
	//~ for (i = 0; i < starpu_memory_node_get_devid(node); i++) {
		//~ data_use_order = data_use_order->next_gpu;
	//~ }
	...
	if (task_currently_treated != NULL && task_currently_treated->cl != NULL) {
		...
		//~ if (STARPU_TASK_GET_HANDLE(task_currently_treated,1) != data_use_order[summed_nb_data_each_gpu[current_gpu]) {
