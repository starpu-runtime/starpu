...
struct starpu_sched_component *starpu_sched_component_darts_create(struct starpu_sched_tree *tree, void *params STARPU_ATTRIBUTE_UNUSED)
{
	...
	if (starpu_cpu_worker_get_count() > 0 && starpu_cuda_worker_get_count() == 0 && starpu_hip_worker_get_count() == 0 && starpu_sycl_worker_get_count() == 0 && starpu_opencl_worker_get_count() == 0 && starpu_mpi_sc_worker_get_count() == 0 && starpu_tcpip_sc_worker_get_count() == 0)
	{
		cpu_only = CPU_ONLY; // Only CPUs
	}
	else if (starpu_cpu_worker_get_count() > 0 && (starpu_cuda_worker_get_count() > 0 || starpu_hip_worker_get_count() == 0 || starpu_sycl_worker_get_count() == 0 || starpu_opencl_worker_get_count() == 0 || starpu_mpi_sc_worker_get_count() == 0 || starpu_tcpip_sc_worker_get_count() == 0))
	{
