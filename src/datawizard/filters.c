...
static void _starpu_data_partition(starpu_data_handle_t initial_handle, starpu_data_handle_t *childrenp, unsigned nparts, struct starpu_data_filter *f, int inherit_state)
{
	...
	for (i = 0; i < nparts; i++)
	{
		...
		/* By default, no scratch data is allocated for the child reduction codelet */
		//child->redux_scratch = NULL; 

		memcpy(child->name, initial_handle->name, sizeof(initial_handle->name));
		_starpu_trace_data_name(child, child->name);
