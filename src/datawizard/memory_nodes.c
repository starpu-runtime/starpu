...
unsigned _starpu_memory_node_register(enum starpu_node_kind kind, int devid)
{
	...
	STARPU_ASSERT_MSG_ALWAYS(node < STARPU_MAXNODES,"Too many nodes (%u) for maximum %d. Use configure option --enable-maxnodes=xxx to update the maximum number of nodes.", node + 1, STARPU_MAXNODES);
...
#undef starpu_node_needs_offset
int starpu_node_needs_offset(unsigned node)
{
	return _starpu_node_needs_offset(node);
}
