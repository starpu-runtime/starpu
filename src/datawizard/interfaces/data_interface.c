...
void _starpu_check_if_valid_and_fetch_data_on_node(starpu_data_handle_t handle, struct _starpu_data_replicate *replicate)
{
	...
	if (valid)
	{
		int ret = _starpu_fetch_data_on_node(handle, handle->home_node, replicate, STARPU_R, 0, 0, 0, NULL, NULL);
		STARPU_ASSERT(!ret);
...
void starpu_data_unregister_submit(starpu_data_handle_t handle)
{
	...
	starpu_data_acquire_on_node_cb(handle, STARPU_ACQUIRE_ALL_NODES, handle->initialized?STARPU_RW:STARPU_W, _starpu_data_unregister_submit_cb, handle);
}
