...
int _starpu_fetch_task_input(struct starpu_task *worker_task, struct _starpu_job *j, int async)
{
	...
	if (async)
	{
		worker->task_transferring = worker_task;
...
void _starpu_fetch_nowhere_task_input(struct _starpu_job *j)
{
	...
	for (index = 0; index < nbuffers; index++)
	{
		...
		if (task->cl->specific_nodes)
			node = STARPU_TASK_GET_NODE(task, descrs[index].index);
		descrs[index].node = node;
		if (node != -1)
			nfetchbuffers++;
	}
