...
static cudaStream_t starpu_cuda_get_out_transfer_stream(int src_devid)
{
	cudaStream_t stream;
...
static cudaStream_t starpu_cuda_get_peer_transfer_stream(int src_devid, int dst_devid)
{
	cudaStream_t stream;
...
void _starpu_cuda_init_worker_binding(struct _starpu_machine_config *config, int no_mp_config STARPU_ATTRIBUTE_UNUSED, struct _starpu_worker *workerarg)
{
	...
	if (_starpu_may_bind_automatically[STARPU_CUDA_WORKER])
	{
		...
		unsigned *preferred_numa_binding = _starpu_get_affinity_vector_by_kind(devid, STARPU_CUDA_RAM);
...
#ifdef STARPU_HAVE_CUDA_MEMCPY_PEER
			if (cuda_peer_enabled[devid][worker->devid])
			{
				cudaDeviceDisablePeerAccess(worker->devid);
				(void) cudaGetLastError();
				cuda_peer_enabled[devid][worker->devid] = 0;
			}
#endif
		}
	}
...
static int _starpu_cuda_driver_init(struct _starpu_worker *worker)
{
	...
	for (i = 1; i < worker_set->nworkers; i++)
		_starpu_trace_worker_init_start(&worker_set->workers[i], STARPU_CUDA_WORKER, 0);
	...
	for (i = 0; i < worker_set->nworkers; i++)
	{
		...
		int rc = _starpu_trace_worker_init_end(worker, STARPU_CUDA_WORKER);
		(void) rc;
	}
	{
		char thread_name[16];
...
#ifdef STARPU_SIMGRID
	static uintptr_t last[STARPU_MAXCUDADEVS];
#ifdef STARPU_DEVEL
...
{
	return starpu_cuda_copy_async_sync_devid(src_ptr,
...
int starpu_cuda_copy_async_sync_devid(void *src_ptr, int src_devid, enum starpu_node_kind src_kind,
...
{
	return starpu_cuda_copy2d_async_sync_devid(src_ptr,
						   starpu_memory_node_get_devid(src_node),
						   starpu_node_get_kind(src_node),
						   dst_ptr,
						   starpu_memory_node_get_devid(dst_node),
...
int starpu_cuda_copy2d_async_sync_devid(void *src_ptr, int src_devid, enum starpu_node_kind src_kind,
					void *dst_ptr, int dst_devid, enum starpu_node_kind dst_kind,
					size_t blocksize,
					size_t numblocks, size_t ld_src, size_t ld_dst,
					cudaStream_t stream, enum cudaMemcpyKind kind)
...
static int _starpu_cuda_copy_interface_from_cuda_to_cuda(starpu_data_handle_t handle, void *src_interface, unsigned src_node, void *dst_interface, unsigned dst_node, struct _starpu_data_request *req)
{
	...
	else
	{
		...
		stream = starpu_cuda_get_peer_transfer_stream(src_devid, dst_devid);
...
static int _starpu_cuda_copy_interface_from_cuda_to_cpu(starpu_data_handle_t handle, void *src_interface, unsigned src_node, void *dst_interface, unsigned dst_node, struct _starpu_data_request *req)
{
	...
	else
	{
		...
		unsigned src_devid = starpu_memory_node_get_devid(src_node);
		stream = starpu_cuda_get_out_transfer_stream(src_devid);
...
static int _starpu_cuda_copy_interface_from_cpu_to_cuda(starpu_data_handle_t handle, void *src_interface, unsigned src_node, void *dst_interface, unsigned dst_node, struct _starpu_data_request *req)
{
	...
	else
	{
		...
		unsigned dst_devid = starpu_memory_node_get_devid(dst_node);
		stream = starpu_cuda_get_in_transfer_stream(dst_devid);
...
static int _starpu_cuda_copy_data_from_cuda_to_cpu(uintptr_t src, size_t src_offset, int src_devid, uintptr_t dst, size_t dst_offset, int dst_devid, size_t size, struct _starpu_async_channel *async_channel)
{
	return starpu_cuda_copy_async_sync_devid((void*) (src + src_offset), src_devid, STARPU_CUDA_RAM,
						 ...
						 size,
						 async_channel?starpu_cuda_get_out_transfer_stream(src_devid):NULL,
...
static int _starpu_cuda_copy_data_from_cuda_to_cuda(uintptr_t src, size_t src_offset, int src_devid, uintptr_t dst, size_t dst_offset, int dst_devid, size_t size, struct _starpu_async_channel *async_channel)
{
	...
	return starpu_cuda_copy_async_sync_devid((void*) (src + src_offset), src_devid, STARPU_CUDA_RAM,
						 ...
						 size,
						 async_channel?starpu_cuda_get_peer_transfer_stream(src_devid, dst_devid):NULL,
...
static int _starpu_cuda_copy_data_from_cpu_to_cuda(uintptr_t src, size_t src_offset, int src_devid, uintptr_t dst, size_t dst_offset, int dst_devid, size_t size, struct _starpu_async_channel *async_channel)
{
	return starpu_cuda_copy_async_sync_devid((void*) (src + src_offset), src_devid, STARPU_CPU_RAM,
						 (void*) (dst + dst_offset), dst_devid, STARPU_CUDA_RAM,
						 size,
						 async_channel?starpu_cuda_get_in_transfer_stream(dst_devid):NULL,
...
static int _starpu_cuda_peer_access(int devid, int peer_devid)
{
	...
	{
		int can;
		cudaError_t cures = cudaDeviceCanAccessPeer(&can, devid, peer_devid);
		(void) cudaGetLastError();
		if (!cures && can)
		{
...
struct _starpu_node_ops _starpu_driver_cuda_node_ops =
{
	...
	.try_enable_peer_access = _starpu_cuda_peer_access,
#endif
};
