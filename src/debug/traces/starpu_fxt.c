	...
	if (task->data)
	{
		...
		for (i = 0; i < task->ndata; i++)
			fprintf(tasks_file, " %s%s%s%s%s%s",
				...
				(task->data[i].mode & STARPU_MPI_REDUX_INTERNAL)?"X-mpi":"",
...
{
	nworkers++;
	STARPU_ASSERT_MSG_ALWAYS(workerid < STARPU_NMAXWORKERS, "Too many workers in this trace (at least %d), please increase in ./configure invocation e.g. with option --enable-maxcpus the maximum number of CPUs and GPUs to the same value as was used for execution", workerid+1);
...
static void thread_set_state(double time, const char *prefix, long unsigned int threadid, const char *name, long job_id)
{
	...
	if (job_id >= 0)
	{
		...
		poti_user_SetState(_starpu_poti_JobState, time, container, "S", name, 1, jobid_str);
	}
	else
		poti_SetState(time, container, "S", name);
#else
	if (job_id >= 0)
		fprintf(out_paje_file, "26	%.9f	%st%lu	S	%s	%ld\n", time, prefix, threadid, name, job_id);
	else
...
static void handle_worker_init_start(struct fxt_ev_native *ev, struct starpu_fxt_options *options)
{
	...
	*/
	...
	do_thread_set_state(now, prefix, threadid, "In", "Runtime", -1);
...
static void handle_checkpoint_begin(struct fxt_ev_native *ev, struct starpu_fxt_options *options)
{
	...
	if (out_paje_file)
	{
		...
		poti_user_NewEvent(_starpu_poti_checkPointState, get_event_time_stamp(ev, options), container, "prog_event", "checkpoint_begin",
				2, ev->param[0], ev->param[1]);
#else
...
static void handle_checkpoint_end(struct fxt_ev_native *ev, struct starpu_fxt_options *options)
{
	...
	if (out_paje_file)
	{
		...
		char container[STARPU_POTI_STR_LEN];
		snprintf(container, sizeof(container), "%sp", options->file_prefix);
		poti_user_NewEvent(_starpu_poti_checkPointState, get_event_time_stamp(ev, options), container, "prog_event", "checkpoint_end",
				2, ev->param[0], ev->param[1]);
#else
