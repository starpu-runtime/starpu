...
do {									\
	...
	{								\
		...
		FUT_DO_PROBE7(_STARPU_FUT_CODELET_DETAILS, ((job)->task)->sched_ctx, __job_size, __job_hash, (job)->task->flops / 1000 / ((job)->task->cl && job->task->cl->type == STARPU_SPMD ? j->task_size : 1), (job)->task->tag_id, workerid, ((job)->job_id)); \
	}								\
} while(0);
...
#define _STARPU_TRACE_TASK_SUBMIT(job, iter, subiter)	\
	FUT_DO_PROBE5(_STARPU_FUT_TASK_SUBMIT, (job)->job_id, iter, subiter, (job)->task->type, _starpu_gettid());
...
#define _STARPU_TRACE_TASK_NAME(a)		do {} while(0)
#define _STARPU_TRACE_TASK_COLOR(a)		do {} while(0)
...
#define _STARPU_TRACE_HANDLE_DATA_UNREGISTER(handle)	do {} while (0)
#define _STARPU_TRACE_DATA_INVALIDATE(handle, node)	do {} while (0)
