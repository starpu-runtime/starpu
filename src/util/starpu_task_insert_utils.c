...
{
	size_t ptr_size;
	STARPU_ASSERT_MSG(state->current_offset + sizeof(size) <= state->arg_buffer_size, "The unpack brings offset %zu beyond the buffer size (%zu)\n", state->current_offset, state->arg_buffer_size);
	memcpy((void *)&ptr_size, state->arg_buffer+state->current_offset, sizeof(ptr_size));
	STARPU_ASSERT_MSG(ptr_size==size, "The given size (%zu) is not the size of the next argument (%zu)\n", size, ptr_size);
	state->current_offset += sizeof(size);

	STARPU_ASSERT_MSG(state->current_offset + size <= state->arg_buffer_size, "The recorded size (%zu) brings beyond the buffer size (%zu)\n", size, state->arg_buffer_size);
...
void starpu_codelet_unpack_arg_fini(struct starpu_codelet_pack_arg_data *state)
{
	...
	{
		_STARPU_MSG("Arguments still need to be unpacked from the starpu_codelet_pack_arg_data (offset %zu - buffer_size %zu)\n", state->current_offset, state->arg_buffer_size);
	}
}
