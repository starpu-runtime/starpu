...
static int _starpu_task_submit_head(struct starpu_task *task)
{
	...
	if (task->cl)
	{
		...
		for (i = 0; i < nbuffers; i++)
		{
			...
			int node = task->cl->specific_nodes ? STARPU_TASK_GET_NODE(task, i) : -1;
			...
			STARPU_ASSERT_MSG(node == STARPU_SPECIFIC_NODE_LOCAL || node == STARPU_SPECIFIC_NODE_CPU || node == STARPU_SPECIFIC_NODE_SLOW || node == STARPU_SPECIFIC_NODE_LOCAL_OR_CPU || node == STARPU_SPECIFIC_NODE_LOCAL_OR_SIMILAR_OR_CPU || node == STARPU_SPECIFIC_NODE_NONE || (node >= 0 && node < (int) starpu_memory_nodes_get_count()), "The codelet-specified memory node does not exist");
...
struct starpu_codelet starpu_codelet_nop =
{
	...
	.sycl_funcs = {starpu_codelet_nop_func},
