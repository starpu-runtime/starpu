						       ...
						       struct _starpu_job *j, unsigned buffer_index)
{
	...
	if (!frozen && ((handle->refcnt == 0) || (!(mode == STARPU_W) && (handle->current_mode == mode))))
	{
		...
		{
			_starpu_data_end_reduction_mode(handle, j?j->task->priority:STARPU_DEFAULT_PRIO);
...
void _starpu_job_set_ordered_buffers(struct _starpu_job *j)
{
	...
	for (i=0 ; i<nbuffers; i++)
	{
		...
		buffers[i].orig_node = task->cl
			? (task->cl->specific_nodes ? STARPU_TASK_GET_NODE(task, i) : -1)
	...
	if (task->cl && task->cl->specific_nodes)
		for (i=1 ; i<nbuffers; i++)
		{
			if (buffers[i].handle == buffers[i-1].handle
				&& STARPU_TASK_GET_NODE(task, buffers[i].index) !=
				   STARPU_TASK_GET_NODE(task, buffers[i-1].index))
			{
